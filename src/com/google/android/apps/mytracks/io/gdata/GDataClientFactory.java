...
public class GDataClientFactory {
	private static final String LOG_TAG = "GDataClientFactory";

	private GDataClientFactory() {
	}

	/**
	 * Creates a new GData client. This factory will fetch the right class for
	 * the platform.
	 * 
	...
	public static GDataClient getGDataClient(Context context) {
		try {
			...
			return new com.google.android.common.gdata.AndroidGDataClient(
					context);
		} catch (LinkageError e) {
			// On all other platforms use the client implementation packaged in
			// the
			// apk.
			Log.i(LOG_TAG, "Using AndroidGDataClient.", e);
