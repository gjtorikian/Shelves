...
public class AndroidGDataClient implements GDataClient {
	 ...
	 * does not allow for changing the URI after creation, e.g., when you want
	...
	private InputStream createAndExecuteMethod(HttpRequestCreator creator,
			String uriString, String authToken) throws HttpException,
			IOException {
		...
		// we follow redirects ourselves, since we want to follow redirects even
		// on
		// POSTs, which
		// the HTTP library does not do. following redirects ourselves also
		// allows
		// us to log
		...
		while (redirectsLeft > 0) {
			...
			// only add the auth token if not null (to allow for GData feeds
			// that do
			// not require
			// authentication.)
			...
			// TODO: handle 301, 307?
			// TODO: let the http client handle the redirects, if we can be sure
			// we'll
			// never get a
			// redirect on POST.
			if (status == 302) {
				...
				try {
					...
					if (Log.isLoggable(TAG, Log.DEBUG)) {
						Log.d(TAG, "Unable to parse " + location.getValue()
								+ " as URI.", use);
						throw new IOException("Unable to parse "
								+ location.getValue() + " as URI.");
					}
					break;
				}
				--redirectsLeft;
	...
	public InputStream getFeedAsStream(String feedUrl, String authToken)
		...
		InputStream in = createAndExecuteMethod(new GetRequestCreator(),
				feedUrl, authToken);
		if (in != null) {
			return in;
		}
	...
	public InputStream createEntry(String feedUrl, String authToken,
		...
		HttpEntity entity = createEntityForEntry(entry,
				GDataSerializer.FORMAT_CREATE);
		InputStream in = createAndExecuteMethod(new PostRequestCreator(
				null /* override */, entity), feedUrl, authToken);
		if (in != null) {
			return in;
		}
	...
	public InputStream updateEntry(String editUri, String authToken,
			GDataSerializer entry) throws HttpException, IOException {
		HttpEntity entity = createEntityForEntry(entry,
				GDataSerializer.FORMAT_UPDATE);
		InputStream in = createAndExecuteMethod(new PostRequestCreator("PUT",
				entity), editUri, authToken);
		if (in != null) {
			return in;
		}
	...
	public void deleteEntry(String editUri, String authToken)
		...
		InputStream in = createAndExecuteMethod(new PostRequestCreator(
				"DELETE", null /* entity */), editUri, authToken);
		if (in == null) {
	...
	public InputStream updateMediaEntry(String editUri, String authToken,
			...
			throws HttpException, IOException {
		InputStream in = createAndExecuteMethod(new MediaPutRequestCreator(
	...
	private HttpEntity createEntityForEntry(GDataSerializer entry, int format)
		...
		if (entryBytes != null && Log.isLoggable(TAG, Log.DEBUG)) {
			try {
				Log.d(TAG, "Serialized entry: "
						+ new String(entryBytes, "UTF-8"));
			} catch (UnsupportedEncodingException uee) {
				// should not happen
