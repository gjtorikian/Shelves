...
class XmlMapsGDataParser extends XmlGDataParser {
	 ...
	 * 
	 * @param plainEntry
	 *            The entry that will be filled.
	 * @throws XmlPullParserException
	 *             Thrown if the XML cannot be parsed.
	 * @throws IOException
	 *             Thrown if the underlying inputstream cannot be read.
	 */
	@Override
	protected void handleEntry(Entry plainEntry) throws XmlPullParserException,
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			...
			case XmlPullParser.START_TAG:
				...
				if ("entry".equals(name)) {
					...
					String rel = parser
							.getAttributeValue(null /* ns */, "rel");
					String type = parser.getAttributeValue(null /* ns */,
							"type");
					String href = parser.getAttributeValue(null /* ns */,
							"href");
					if ("edit".equals(rel)) {
					...
					String category = parser.getAttributeValue(null /* ns */,
							"term");
					if (category != null && category.length() > 0) {
						entry.setCategory(category);
					}
					String categoryScheme = parser.getAttributeValue(
							null /* ns */, "scheme");
					...
					entry.setPrivacy("yes".equals(draft) ? "unlisted"
							: "public");
