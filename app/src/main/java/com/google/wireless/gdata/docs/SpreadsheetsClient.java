...
public class SpreadsheetsClient extends GDataServiceClient {
	 ...
	 * 
	 * @param client
	 *            The GDataClient that should be used to authenticate requests,
	 *            retrieve feeds, etc
	 */
	 ...
	 *            the Class of entry type that will be parsed, which lets this
	 *            method figure out which parser to create
	 * @param feedUri
	 *            the URI of the feed to be fetched and parsed
	 * @param authToken
	 *            the current authToken to use for the request
	 * @return a parser for the indicated feed
	 * @throws AuthenticationException
	 *             if the authToken is not valid
	 * @throws ParseException
	 *             if the response from the server could not be parsed
	 */
	private GDataParser getParserForTypedFeed(
			Class<? extends Entry> feedEntryClass, String feedUri,
			String authToken) throws AuthenticationException, ParseException,
			IOException {
	...
	/**
	 * Converts an HTTP exception that happened while reading into the
	 * equivalent local exception.
	 */
	...
	@Override
	public GDataParser getParserForFeed(Class feedEntryClass, String feedUri,
			String authToken) throws ParseException, IOException {
		try {
	 ...
	 * 
	 * @param entry
	 *            the entry to be updated; must include its URI
	 * @param authToken
	 *            the current authToken to be used for the operation
	 * @return An Entry containing the re-parsed version of the entry returned
	 *         by the server in response to the update
	 * @throws ParseException
	 *             if the server returned an error, if the server's response was
	 *             unparseable (unlikely), or if <code>entry</code> is of a
	 *             read-only type
	 * @throws IOException
	 *             on network error
	 */
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private void convertHttpExceptionForWrites(Class entryClass,
			String message, HttpException cause) throws ParseException,
			IOException {
		switch (cause.getStatusCode()) {
